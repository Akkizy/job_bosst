...
// ─── SerpApi Google Jobs (Catho, InfoJobs, Indeed, LinkedIn, Trampos) ────
export async function fetchSerpApi(keyword, market) {
  ...
  if (market === 'nacional' || market === 'ambos') {
    queries.push({ q: keyword, location: 'Brasil', gl: 'br', hl: 'pt' })
  }
  if (market === 'internacional' || market === 'ambos') {
    queries.push({ q: keyword, location: '', gl: 'us', hl: 'en' })
  }

  for (const { q, location, gl, hl } of queries) {
    try {
      const params = new URLSearchParams({
        engine: 'google_jobs',
        q: q || '',
      ...
      const json = await res.json()
      for (const item of json.jobs_results || []) {
        const isBR = gl === 'br'
        const loc = (item.location || '').toLowerCase()
        const isRemote = ['remoto', 'remote', 'home office', 'anywhere'].some(w => loc.includes(w))
        const source = item.via?.replace('via ', '') || 'Google Jobs'
        jobs.push({
          id: `serpapi-${Buffer.from((item.job_id || item.title + item.company_name).slice(0,40)).toString('base64').slice(0,24)}`,
          title: item.title,
          company: item.company_name || 'N/A',
          url: item.related_links?.[0]?.link || item.share_link || '',
          salary_min: null,
          remote: isRemote,
          location: item.location || (isBR ? 'Brasil' : 'Internacional'),
          source,
    ...
    } catch (e) { console.error('Erro SerpApi:', e.message) }
  }
  return jobs
}
...
export async function fetchAllJobs(keyword, market = 'ambos') {
  // SerpApi (Google Jobs) + CareerJet + Adzuna + Jooble cobrem TODAS as áreas
  const fetchers = [
    fetchSerpApi(keyword, market),   // Google Jobs: Catho, InfoJobs, Indeed, LinkedIn, Trampos
